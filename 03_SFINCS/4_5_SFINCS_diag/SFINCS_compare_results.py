...
VOL_TOL_PCT   = 1.0      # steady if |dV/dt| < this % of total inflow
...
def total_inflow(root):
    if Q_IN_M3S is not None:
        return float(Q_IN_M3S)
    dis = root / "sfincs.dis"
    if not dis.exists():
        return None
    last = None
    for ln in dis.read_text().splitlines():
        parts = ln.split()
        try:
        ...
        if len(vals) >= 2:
            last = vals
    return float(sum(last[1:])) if last else None
...
def process_run(name, root):
    ...
    stats = {
        ...
        "volume_mil_m3": float(np.nansum(hmax) * area_m2 / 1e6)
    ...
    for t in range(zs.shape[0]):
        ...
        vol = float(np.nansum(np.where(wet_t, depth_t, 0.0)) * area_m2)
        rows.append({"run": name, "hour": t,
                     "wet_area_km2": float(wet_t.sum() * area_m2 / 1e6),
                     "volume_mil_m3": vol / 1e6,
                     "dz_median_m": dmed, "dz_max_m": dmax})
    df = pd.DataFrame(rows)
    df["dV_dt_m3s"] = df["volume_mil_m3"].diff() * 1e6 / 3600.0
    q_in = total_inflow(root)
    df["dV_dt_pct_inflow"] = (df["dV_dt_m3s"] / q_in * 100.0) \
        if q_in else np.nan
    last = df.iloc[-1]
    ...
    stats["q_in_m3s"] = q_in
    stats["last_hour_dV_dt_m3s"] = float(df["dV_dt_m3s"].iloc[-1])
    stats["last_hour_dV_dt_pct_inflow"] = (
        float(df["dV_dt_pct_inflow"].iloc[-1]) if q_in else None)
    if q_in:
        stats["steady"] = bool(abs(stats["last_hour_dV_dt_pct_inflow"])
                               < VOL_TOL_PCT)
        crit = f"dV/dt {stats['last_hour_dV_dt_pct_inflow']:.2f} % of inflow"
    else:
        stats["steady"] = bool(last["dz_max_m"] < STEADY_DZ_M)
        crit = f"dz_max {stats['last_hour_dz_max_m']:.3f} m (no inflow found)"
    stats["steady_cellmax"] = bool(last["dz_max_m"] < STEADY_DZ_M)
    print(f"  {name}: wet {stats['wet_area_km2']:.1f} km2, {crit} -> "
