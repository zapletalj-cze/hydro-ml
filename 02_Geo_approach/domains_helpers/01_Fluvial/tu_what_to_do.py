...
TUFLOW_server = "D_local"  ## nickname of th TUFLOW server - where domain will be send for computation
...
tu_what_to_do = {
    ...
    "Q_Do_BC_Dbase_local": "n",  ## create copy of the hydrology into given domain, the hydrology inputs must exist in the CommonInput folder on the source-drive
    ...
    #
    "Q_Do_CopyDomains_to_server": "n",  ## copy all domains to chosen server for the computation
    "Q_Do_Create_Scenario": "n",  ## crete a scenario to start the computation on the server
    "Q_Do_Create_YAML": "y",  ## create per-domain YAML file for the computation server (yaml_project, yaml_peril_path etc. must be set in main_setup_fluvial.yaml)
}
