...
def skeleton_to_linestrings(skeleton_mask, geotransform):
    """
    Convert binary skeleton mask to a list of LineStrings in world coordinates.
    Each unit pixel-step becomes a 1-segment LineString; shapely linemerge then
    glues them into longer polylines.
    """
    ...
    origin_x, pixel_w, _, origin_y, _, pixel_h_neg = geotransform  # pixel_h_neg < 0
    pixel_h = -pixel_h_neg
    ...
    for r, c in coord_set:
        for dr, dc in [(0, 1), (1, 0), (1, 1), (1, -1)]:  # forward neighbors only (no duplicates)
            nr, nc = r + dr, c + dc
