...
# Spatial reference
# ------------------------------------------------------------
TARGET_CRS = "EPSG:3035"  # ETRS89 / ETRS-LAEA, common for European spatial data
NETHERLANDS_BBOX_WGS84 = (3.3, 50.7, 7.2, 53.6)
...
def segment_line(line, segment_length, min_length):
    ...
    if math.isnan(line.length) or line.length < min_length:
        return []
...
if __name__ == "__main__":
    ...
    gdf_levees = load_bdot_levees(LEVEES_GPKG, TARGET_CRS)
    gdf_reaches = load_merit_basins(
        MERIT_BASINS_SHP, NETHERLANDS_BBOX_WGS84, TARGET_CRS
    )
