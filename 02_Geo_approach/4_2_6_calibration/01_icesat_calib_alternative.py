...
def main():
    ...
    # datum offset from ALL reference points on a 10 km grid (no sampling)
    dtm = DtmSampler(DTM_TIF)
    from shapely.ops import unary_union, nearest_points
    from shapely.geometry import Point as _P
    axis = unary_union(list(seg.geometry.values))

    # offset uses EVERY point of the export, no levee/vegetation exclusion:
    # the median is robust and levee-proximate points are ~1 % of the mass
    ...
    order = np.argsort(cell_of, kind="stable")   # cell-sorted reads: cache-friendly
    diffs = np.full(len(ref_idx), np.nan)
    print(f"offset: reading DTM at {len(ref_idx)} points (ALL, no filtering) ...")
