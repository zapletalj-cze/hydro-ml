...
OUTPUT_GPKG = Path(r"D:\90_PersonalFoldlers\JZa\DataProcessing\levees_detection\sat_lidar\01_data\ICE_SAT\ATL08\atl08_terrain_heights_updated.gpkg")
...
def extract_terrain_heights(h5_path: Path, aoi_polygon: Polygon) -> list:
    ...
    with h5py.File(h5_path, "r") as f:
        for beam in BEAMS:
            ...
            if FILTER_CLOUDS:
                # Measured on a real v007 granule: layer_flag != 0 for ALL
                # 100k segments (0.0 % pass), while cloud_flag_atm <= 1 passes
                # ~91 %. layer_flag is over-conservative and would zero the
                # dataset, so cloudiness is filtered via cloud_flag_atm only;
                # layer_flag stays recorded in the output for later analysis.
                valid &= np.where(np.isfinite(cloud), cloud <= 1, True)
