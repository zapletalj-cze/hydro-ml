...
def figure_training_curves(hist, out_dir):
    ...
    ax = axes[0]
    ax.plot(hist["epoch"], hist["train_loss"], color=C_TRAIN, linewidth=2.1,
            solid_capstyle="round", label="trénink")
    ax.plot(hist["epoch"], hist["val_loss"], color=C_VAL, linewidth=2.1,
            linestyle=(0, (5, 2)), solid_capstyle="round", label="validace")
    ax.axvline(best_epoch, color=SECOND, linestyle=":", linewidth=1.1)
    ...
    ax = axes[1]
    ax.plot(hist["epoch"], hist["val_dice"], color=C_DICE, linewidth=2.1,
            solid_capstyle="round", label="Dice")
    ax.plot(hist["epoch"], hist["val_cldice"], color=C_CLDICE, linewidth=2.1,
