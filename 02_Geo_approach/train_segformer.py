...
# ------- Paths ----------------------------------------------
PATCHES_DIR_PL = Path(
    r"D:\90_PersonalFoldlers\JZa\DataProcessing\levees_detection\geomorphological_ML\patches_v01_PL\patches"
)
PATCHES_DIR_NL = Path(
    r"D:\90_PersonalFoldlers\JZa\DataProcessing\levees_detection\geomorphological_ML\patches_v01_NL\patches"
)
METADATA_CSV_PL = Path(
    r"D:\90_PersonalFoldlers\JZa\DataProcessing\levees_detection\geomorphological_ML\patches_v01_PL\patches_metadata.csv"
)
METADATA_CSV_NL = Path(
    r"D:\90_PersonalFoldlers\JZa\DataProcessing\levees_detection\geomorphological_ML\patches_v01_NL\patches_metadata.csv"
)

# Output dir - MUST be unique per variant
OUTPUT_DIR = Path(
    r"D:\90_PersonalFoldlers\JZa\DataProcessing\levees_detection\geomorphological_ML\training_v03_segformer_v3"
)
...
def load_and_split_metadata():
    ...
    df["split"] = df["source_idx_global"].apply(
        lambda s: (
            "train" if s in train_sources else "val" if s in val_sources else "test"
        )
    )
    ...
    logging.info(
        f"First test patch_ids (sanity check): {df_test['patch_id'].head().tolist()}"
    )
...
def compute_or_load_norm_stats(df_train):
    ...
    for _, row in tqdm(
        ...
        npz_path = {"PL": PATCHES_DIR_PL, "NL": PATCHES_DIR_NL}[
            row["region"]
        ] / f"{row['patch_id']}.npz"
    ...
    norm_stats["dsm"] = {
        "mean": 0.0,
        "std": 1.0,
    }  # sentinel; DSM is per-patch normalized
...
def train(model, train_loader, val_loader):
    ...
    for epoch in range(N_EPOCHS):
        # --- Train epoch ---
        model.train()
        train_loss_sum, train_dice_sum, train_cldice_sum, n_train_batches = (
            0.0,
            0.0,
            0.0,
            0,
        ...
        for batch in tqdm(train_loader, desc=f"Epoch {epoch+1}/{N_EPOCHS} [train]"):
            ...
            with torch.no_grad():
                ...
                train_dice_sum += (
                    ((2 * inter_t + 1e-6) / (union_t + 1e-6)).mean().item()
                )

                skel_p_t = soft_skeleton(pred_bin_t, CLDICE_ITER)
                skel_l_t = soft_skeleton(label, CLDICE_ITER)
                tprec_t = ((skel_p_t * label).sum(dim=(1, 2, 3)) + 1e-6) / (
                    skel_p_t.sum(dim=(1, 2, 3)) + 1e-6
                )
                trec_t = ((pred_bin_t * skel_l_t).sum(dim=(1, 2, 3)) + 1e-6) / (
                    skel_l_t.sum(dim=(1, 2, 3)) + 1e-6
                )
                train_cldice_sum += (
                    (2 * tprec_t * trec_t / (tprec_t + trec_t)).mean().item()
                )
...
def plot_training_curves(history):
    ...
    axes[0, 0].plot(
        hist_df["epoch"], hist_df["val_loss"], label="Val loss", linewidth=2
    )
    axes[0, 0].axvline(
        best_epoch,
        color="grey",
    ...
    axes[1, 1].plot(
        hist_df["epoch"],
        hist_df["train_dice"] - hist_df["val_dice"],
        color="tab:green",
        linewidth=2,
        label="Dice gap",
    )
    axes[1, 1].plot(
        hist_df["epoch"],
        hist_df["train_cldice"] - hist_df["val_cldice"],
        color="tab:purple",
        linewidth=2,
        label="clDice gap",
    )
...
def build_metrics_summary(
    history, checkpoint, val_results, test_results, run_start_time
):
    ...
    summary = {
        "run_info": {
            ...
            "segformer_backbone": (
                SEGFORMER_BACKBONE if ARCHITECTURE == "segformer" else None
            ),
...
def evaluate_split(model, df_split, split_name, norm_stats, patches_root_dirs):
    ...
    ds = LeveeDataset(
        df_split, patches_root_dirs, INPUT_CHANNELS, norm_stats, augment=False
    )
    loader = DataLoader(
        ds,
        batch_size=BATCH_SIZE,
