...
def read_scene_footprint(path: Path):
    ...
    translate_opts = gdal.TranslateOptions(
        format="VRT", widthPct=2.0, heightPct=2.0, resampleAlg=gdal.GRA_NearestNeighbour
    )
    ...
    footprint_opts = gdal.FootprintOptions(format="MEM", maxPoints=20)
...
def build_tile_index(directory: Path, polarization: str) -> gpd.GeoDataFrame:
    ...
    if not records:
        return gpd.GeoDataFrame(
            columns=["path", "name", "geometry"], crs=f"EPSG:{EPSG}"
        )
